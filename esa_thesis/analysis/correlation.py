...
Run:
    python -m esa_thesis correlation
    python -m esa_thesis correlation --max_rows 500000  # faster, uses a sample
    python -m esa_thesis correlation --min_corr 0.7     # tighter cluster threshold
"""
...
# ── config ─────────────────────────────────────────────────────────────────────
TRAIN_FILE  = Path(str(project_path('data/preprocessed/multivariate/ESA-Mission1-semi-supervised/84_months.train.csv')))
LABELS_FILE = Path(str(project_path('data/ESA-Mission1/labels.csv')))
OUT_DIR     = Path(str(project_path('results_longrun/channel_correlation')))
